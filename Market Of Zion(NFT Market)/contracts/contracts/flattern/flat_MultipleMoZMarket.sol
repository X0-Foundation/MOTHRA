...
/** 
 *  SourceUnit: d:\GitWork\TheXdaoNFT\contracts\contracts\MultipleTheXdaoMarket.sol
*/
...
contract MultipleTheXdaoNFT is ERC1155, AccessControl {
    ...
    constructor() ERC1155("MultipleTheXdaoNFT") {
        factory = msg.sender;
    }
	 ...
	 */
    function addMultiItem( uint256 supply, uint256 royalty, string memory _uri ) external returns (uint256) {
...
////import "./MultipleTheXdaoNFT.sol";
////import "./IBEP20.sol";

interface IMultipleTheXdaoNFT {
	function initialize(string memory _name, string memory _uri, address creator, bool bPublic) external;
...
contract MultipleTheXdaoMarket is Ownable, ERC1155Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;

	uint256 constant public PERCENTS_DIVIDER = 1000;
	uint256 constant public swapFee = 25;	
	...
	uint256 public currentPairId = 100000000000;
	...
	constructor (address _governanceToken, address _feeAddress) {		
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) external returns(address collection) {
		bytes memory bytecode = type(MultipleTheXdaoNFT).creationCode;
        ...
        IMultipleTheXdaoNFT(collection).initialize(_name, _uri, msg.sender, bPublic);
		collections.push(collection);
		emit MultiCollectionCreated(collection, msg.sender, _name, _uri, bPublic);
	}

    function multipleList(address _collection, uint256 _tokenId, uint256 _amount, uint256 _price) external {
		require(_price > 0, "invalid price");
		require(_amount > 0, "invalid amount");
		IMultipleTheXdaoNFT nft = IMultipleTheXdaoNFT(_collection);
	...
	function multipleDelist(uint256 _id) external {
		...
		IMultipleTheXdaoNFT(pairs[_id].collection).safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].tokenId, pairs[_id].balance, "delist Marketplace");
		pairs[_id].balance = 0;
    ...
    function multipleBuy(uint256 _id, uint256 _amount) external {
		...
		if(swapFee > 0) {
			require(governanceToken.transfer(feeAddress, tokenAmount.mul(swapFee).div(PERCENTS_DIVIDER)));		
		}
		// transfer governance token to creator
		if(item.creatorFee > 0) {
			require(governanceToken.transfer(item.creator, tokenAmount.mul(item.creatorFee).div(PERCENTS_DIVIDER)));
		}
		// transfer governance token to owner
		uint256 ownerPercent = PERCENTS_DIVIDER.sub(swapFee).sub(item.creatorFee);
		require(governanceToken.transfer(item.owner, tokenAmount.mul(ownerPercent).div(PERCENTS_DIVIDER)));
		
		// transfer NFT token to buyer
		IMultipleTheXdaoNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, item.tokenId, _amount, "buy from Marketplace");
