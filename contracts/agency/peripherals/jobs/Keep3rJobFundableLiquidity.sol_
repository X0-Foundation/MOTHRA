...
abstract contract Keep3rJobFundableLiquidity is IKeep3rJobFundableLiquidity, ReentrancyGuard, Keep3rJobOwnership, Keep3rParameters {
  ...
  /// @inheritdoc IKeep3rJobFundableLiquidity
  // Mike: rewardPeriod/inflationPeriod * KP3R averaged between last and current periods, sumed up for all liquidities of the job.
  function jobPeriodCredits(address _job) public view override returns (uint256 _periodCredits) {
    for (uint256 i; i < _jobLiquidities[_job].length(); i++) {
      ...
      if (_approvedLiquidities.contains(_liquidity)) {
        ...
        if (_tickCache.period != 0) { // if successful
  ...
  /// @inheritdoc IKeep3rJobFundableLiquidity
  // If rewarded at within one period past, return tweaked _liquidityCredits[_job], else return current twap.
  ...
  function totalJobCredits(address _job) external view override returns (uint256 _credits) {
    ...
    if ((rewardedAt[_job] > _period(block.timestamp - rewardPeriodTime))) { // (last, ...)
      // Will calculate cooldown if it outdated
      if ((block.timestamp - rewardedAt[_job]) >= rewardPeriodTime) { // rewardedAt in (last, ts - period]. Skip one period.
        // Will calculate cooldown from last reward reference in this period
        _cooldown -= (rewardedAt[_job] + rewardPeriodTime);
      } else {                                                          // rewardedAt in (ts - period, ts]. No skip.
        // Will calculate cooldown from last reward timestamp
        _cooldown -= rewardedAt[_job];
      }
    } else {                                                            // rewarded in (-infinite, last]. Start from the current.
  ...
  /// @inheritdoc IKeep3rJobFundableLiquidity
  // Mike: Choose the last and current period points, and get the difference of tick * time accumulate between the period points.
  ...
  // Mike: Make sure of block.timestamp - rewardedAt[_job] <= rewardPeriodTime. Lagging behind last else lagging over a period else lagging less than a period.
  function _updateJobCreditsIfNeeded(address _job) internal returns (bool _rewarded) {
    if (rewardedAt[_job] < _period(block.timestamp)) {
      ...
      if (rewardedAt[_job] <= _period(block.timestamp - rewardPeriodTime)) { // rewardedAt <= last. Reward at current.
        // Will reset job to period syncronicity if a full period passed without rewards
        _updateJobPeriod(_job);
        // _jobPeriodCredits[_job] is now the rewardPeriodTime/inflationTime * KP3R averaged on [last period, current period]
        _jobLiquidityCredits[_job] = _jobPeriodCredits[_job];
        rewardedAt[_job] = _period(block.timestamp); 
        _rewarded = true;

        // =============== 0 + full phase is finished, at the current period, after a period or more, starting a new phase on top of jobPeriodCredit.

      } else if ((block.timestamp - rewardedAt[_job]) >= rewardPeriodTime) { // last < rewardedAt <= ts - period. Reward a period later.
        ...
        // =============== 0 + full phase is finished, at later than the current period, after exactly a period, starting a new phase on top of jobPeriodCredit.

      } else if (workedAt[_job] < _period(block.timestamp)) { // ts - period < rewardedAt <= ts. 
        // First keeper on period has to update job accountance to current twaps
        uint256 previousPeriodCredits = _jobPeriodCredits[_job]; // Sure it's not zero? =============================
        _updateJobPeriod(_job);   // 
        ...
        // ================ the existing phase is ongoing, with a proportional tweak, which started before less than a period and before the current period.
      }
    }
  }
  ...
  // Mike: find the rewardPeriodTime/inflationTime * KP3R averaged between the last and current period points.
  // _tick is updated to the current period point.
