...
contract ControlCenter is IControlCenter, Node, Ownable {
    ...
    function ruleOutDeviatedPrice(bool isNichePair, PairSnapshot memory pairSnapshot) external override virtual {
        (pairSnapshot.reserve0, pairSnapshot.reserve1, ) = IXPair(pairSnapshot.pair).getReserves();
        if (isNichePair) {
    ...
    function captureInitialPairState(
        ...
        (pairSnapshot.token0, pairSnapshot.token1) = (IXPair(pairSnapshot.pair).token0(), IXPair(pairSnapshot.pair).token1());
