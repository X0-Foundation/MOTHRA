...
contract CompoundBurnE is Ownable {
    ...
    //==================== ERC20 core data ====================
    string private constant _name = "CompoundBurnE";
    string private constant _symbol = "CEBNE";
    ...
    function _viewTotalPendingReward() internal view returns (uint) {
        ...
        uint extraBlocks = nowBlock - latestBlock;
        (uint numerator, uint denominator) = analyticMath.pow(MAGNIFIER - DecPerCycle, MAGNIFIER, extraBlocks, CYCLE);
        uint extraPending = _totalSupply - IntegralMath.mulDivC(_totalSupply, numerator, denominator);
        return extraPending;
    }
