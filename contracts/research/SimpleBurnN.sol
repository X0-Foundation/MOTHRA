...
contract SimpleBurnN is Ownable {
    ...
    function _viewUserPendingReward(address user) internal view returns (uint pending) {
        ...
        if (missings > 0) {
            ...
            uint pending;
            if (block.number % 2 == 0) {
                pending = _balances[user] - IntegralMath.mulDivF(_balances[user], numerator, denominator);
            } else {
                pending = _safeSubtract(_balances[user], IntegralMath.mulDivC(_balances[user], numerator, denominator));
            }
        }
    }
    ...
    function _viewTotalPendingReward() internal view returns (uint pending) {
        ...
        if (missings > 0) {
            (uint numerator, uint denominator) = analyticMath.pow(MAGNIFIER - DecPerCycle, MAGNIFIER, missings, CYCLE);
            pending = _safeSubtract(latestNet, IntegralMath.mulDivF(VIRTUAL, numerator, denominator));
        }
    }
