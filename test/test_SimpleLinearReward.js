...
const {
    deployWireLibrary, deploySimpleLinearReward,
...
async function showTotalState() {
    const s = await SimpleLinearReward.getTotalState();
...
async function showUserState(user) {
    const s = await SimpleLinearReward.getUserState(user.address);
...
async function checkConsistency() {
    report = await SimpleLinearReward.checkForConsistency();
...
async function transfer(sender, recipient, amount) {
    ...
    let balance = await SimpleLinearReward.balanceOf(sender.address);
    ...
    let symbol = await SimpleLinearReward.symbol();
    //console.log("\t%s is transferring %s %s SLRW ...".yellow, sender.name, recipient.name, weiToEth(amountWei));
    console.log("\t%s is transferring %s SLRW to %s...".yellow, 
    ...
    tx = SimpleLinearReward.connect(sender).transfer(recipient.address, amountWei );
    (await tx).wait();
...
async function mint(minter, to, amount) {
    let amountWei = ethToWei(amount);
    await console.log("\t%s is minting %s SLRW to %s ...".yellow, 
    minter.name == undefined ? "undefined" : minter.name,
    amount,
    to.hasOwnProperty("name") ? (to.name == undefined ? "undefined" : to.name) : "NoName" );

    tx = SimpleLinearReward.connect(minter).mint(to.address, amountWei );
    (await tx).wait();
...
async function burn(burner, from, amount) {
    ...
    let amountWei = ethToWei(amount);
    let balance = await SimpleLinearReward.balanceOf(from.address);
    if (amount > weiToEth(balance) * minOneBlockSurvival) {
        amount = Math.floor((weiToEth(balance) * minOneBlockSurvival)*100)/100;
    }
    amountWei = ethToWei(amount);
    await console.log("\t%s is burning %s SLRW from %s ...".yellow, 
    burner.name == undefined ? "undefined" : burner.name,
    amount,
    from.hasOwnProperty("name") ? (from.name == undefined ? "undefined" : from.name) : "NoName" );

    tx = SimpleLinearReward.connect(burner).burn(from.address, amountWei );
    (await tx).wait();
...
describe("====================== Stage 1: Deploy ======================\n".yellow, async function () {
    it("1.1 Main contracts are deployed.\n".green, async function () {
        ...
        SimpleLinearReward = await deploySimpleLinearReward(owner, analyticMath.address);
        console.log("\tSimpleLinearReward contract deployed at: %s", SimpleLinearReward.address);
        console.log("\tOwner's balance: %s", await SimpleLinearReward.balanceOf(owner.address));
    ...
    it("1.2 DistTyepA token name, symbol and decimals were checked.\n".green, async function () {
      const name = await SimpleLinearReward.name();
      console.log("\tSimpleLinearReward token's name: %s", name);
      expectEqual(name, "SimpleLinearReward");

      const symbol = await SimpleLinearReward.symbol();
      console.log("\tSimpleLinearReward symbol: %s", symbol);
      expectEqual(symbol, "SLRW");

      const decimals = await SimpleLinearReward.decimals();
      console.log("\tSimpleLinearReward decimals: %s", decimals);
    ...
    it("1.3 Total supply and owner balance of SLRW are checked.\n".green, async function () {
      const totalSupply = await SimpleLinearReward.totalSupply();
      console.log("\tSimpleLinearReward total supply: %s gways",BigInt(totalSupply));
      expectEqual(weiToEth(totalSupply), weiToEth(INITIAL_SUPPLY));

      console.log("\tTotal supply amount was minted to owner.");
      const ownerTgrBalance = await SimpleLinearReward.balanceOf(owner.address);
      console.log("\tSimpleLinearReward owner balance: %s gways", BigInt(ownerTgrBalance));
    ...
    it("1.4 Allowance control functions were checked.\n".green, async function () {
      console.log("Owner approves Alice to spend 1000 SLRW");
      await SimpleLinearReward.connect(owner).approve(alice.address, ethToWei(1000));

      let allowanceOwnerAlice = await SimpleLinearReward.allowance(owner.address, alice.address);
      ...
      await SimpleLinearReward.connect(owner).increaseAllowance(alice.address, ethToWei(1000));
      allowanceOwnerAlice = await SimpleLinearReward.allowance(owner.address, alice.address);
      console.log("\tallowance[owner][alice]: %s", weiToEthEn(allowanceOwnerAlice));
      expectEqual(weiToEth(allowanceOwnerAlice), 2000);

      console.log("\tDecrease it by 1000.");
      await SimpleLinearReward.connect(owner).decreaseAllowance(alice.address, ethToWei(1000));
      allowanceOwnerAlice = await SimpleLinearReward.allowance(owner.address, alice.address);
      console.log("\tallowance[owner][alice]: %s", weiToEthEn(allowanceOwnerAlice));
      expectEqual(weiToEth(allowanceOwnerAlice), 1000);
