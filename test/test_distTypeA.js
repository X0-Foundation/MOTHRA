...
const {
    deployWireLibrary, deployDistTypeA,
...
async function showTotalState() {
    const s = await distTypeA.getTotalState();
...
async function showUserState(user) {
    const s = await distTypeA.getUserState(user.address);
...
async function checkConsistency() {
    report = await distTypeA.checkForConsistency();
...
async function transfer(sender, recipient, amount) {
    ...
    let balance = await distTypeA.balanceOf(sender.address);
    if (amountWei > balance) {
        amountWei = balance;
        amount = weiToEth(amountWei);
    }
    let symbol = await distTypeA.symbol();
    //console.log("\t%s is transferring %s %s TY_A ...".yellow, sender.name, recipient.name, weiToEth(amountWei));
    console.log("\t%s is transferring %s TY_A to %s...".yellow, 
    ...
    tx = distTypeA.connect(sender).transfer(recipient.address, amountWei );
    (await tx).wait();
...
describe("====================== Stage 1: Deploy ======================\n".yellow, async function () {
    it("1.1 Main contracts are deployed.\n".green, async function () {
        ...
        distTypeA = await deployDistTypeA(owner, analyticMath.address);
        console.log("\tDistTypeA contract deployed at: %s", distTypeA.address);
        console.log("\tOwner's balance: %s", await distTypeA.balanceOf(owner.address));
    ...
    it("1.2 DistTyepA token name, symbol and decimals were checked.\n".green, async function () {
      const name = await distTypeA.name();
      console.log("\tDistTypeA token's name: %s", name);
      expectEqual(name, "DistTypeA");

      const symbol = await distTypeA.symbol();
      console.log("\tDistTypeA symbol: %s", symbol);
      expectEqual(symbol, "TY_A");

      const decimals = await distTypeA.decimals();
      console.log("\tDistTypeA decimals: %s", decimals);
    ...
    it("1.3 Total supply and owner balance of TY_A are checked.\n".green, async function () {
      const totalSupply = await distTypeA.totalSupply();
      console.log("\tDistTypeA total supply: %s gways",BigInt(totalSupply));
      expectEqual(weiToEth(totalSupply), weiToEth(INITIAL_SUPPLY));

      console.log("\tTotal supply amount was minted to owner.");
      const ownerTgrBalance = await distTypeA.balanceOf(owner.address);
      console.log("\tDistTypeA owner balance: %s gways", BigInt(ownerTgrBalance));
    ...
    it("1.4 Allowance control functions were checked.\n".green, async function () {
      console.log("Owner approves Alice to spend 1000 TY_A");
      await distTypeA.connect(owner).approve(alice.address, ethToWei(1000));

      let allowanceOwnerAlice = await distTypeA.allowance(owner.address, alice.address);
      ...
      await distTypeA.connect(owner).increaseAllowance(alice.address, ethToWei(1000));
      allowanceOwnerAlice = await distTypeA.allowance(owner.address, alice.address);
      console.log("\tallowance[owner][alice]: %s", weiToEthEn(allowanceOwnerAlice));
      expectEqual(weiToEth(allowanceOwnerAlice), 2000);

      console.log("\tDecrease it by 1000.");
      await distTypeA.connect(owner).decreaseAllowance(alice.address, ethToWei(1000));
      allowanceOwnerAlice = await distTypeA.allowance(owner.address, alice.address);
      console.log("\tallowance[owner][alice]: %s", weiToEthEn(allowanceOwnerAlice));
      expectEqual(weiToEth(allowanceOwnerAlice), 1000);
