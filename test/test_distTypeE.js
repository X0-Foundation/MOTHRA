...
const {
    deployWireLibrary, deployDistTypeE,
...
async function showTotalState() {
    const s = await distTypeE.getTotalState();
...
async function showUserState(user) {
    const s = await distTypeE.getUserState(user.address);
...
async function checkConsistency() {
    report = await distTypeE.checkForConsistency();
...
async function transfer(sender, recipient, amount) {
    ...
    let balance = await distTypeE.balanceOf(sender.address);
    if (amountWei > balance) {
        amountWei = balance;
        amount = weiToEth(amountWei);
    }
    let symbol = await distTypeE.symbol();
    //console.log("\t%s is transferring %s %s TY_E ...".yellow, sender.name, recipient.name, weiToEth(amountWei));
    console.log("\t%s is transferring %s TY_E to %s...".yellow, 
    ...
    tx = distTypeE.connect(sender).transfer(recipient.address, amountWei );
    (await tx).wait();
...
describe("====================== Stage 1: Deploy ======================\n".yellow, async function () {
    it("1.1 Main contracts are deployed.\n".green, async function () {
        ...
        distTypeE = await deployDistTypeE(owner, analyticMath.address);
        console.log("\tDistTypeE contract deployed at: %s", distTypeE.address);
        console.log("\tOwner's balance: %s", await distTypeE.balanceOf(owner.address));
    ...
    it("1.2 DistTyepA token name, symbol and decimals were checked.\n".green, async function () {
      const name = await distTypeE.name();
      console.log("\tDistTypeE token's name: %s", name);
      expectEqual(name, "DistTypeE");

      const symbol = await distTypeE.symbol();
      console.log("\tDistTypeE symbol: %s", symbol);
      expectEqual(symbol, "TY_E");

      const decimals = await distTypeE.decimals();
      console.log("\tDistTypeE decimals: %s", decimals);
    ...
    it("1.3 Total supply and owner balance of TY_E are checked.\n".green, async function () {
      const totalSupply = await distTypeE.totalSupply();
      console.log("\tDistTypeE total supply: %s gways",BigInt(totalSupply));
      expectEqual(weiToEth(totalSupply), weiToEth(INITIAL_SUPPLY));

      console.log("\tTotal supply amount was minted to owner.");
      const ownerTgrBalance = await distTypeE.balanceOf(owner.address);
      console.log("\tDistTypeE owner balance: %s gways", BigInt(ownerTgrBalance));
    ...
    it("1.4 Allowance control functions were checked.\n".green, async function () {
      console.log("Owner approves Alice to spend 1000 TY_E");
      await distTypeE.connect(owner).approve(alice.address, ethToWei(1000));

      let allowanceOwnerAlice = await distTypeE.allowance(owner.address, alice.address);
      ...
      await distTypeE.connect(owner).increaseAllowance(alice.address, ethToWei(1000));
      allowanceOwnerAlice = await distTypeE.allowance(owner.address, alice.address);
      console.log("\tallowance[owner][alice]: %s", weiToEthEn(allowanceOwnerAlice));
      expectEqual(weiToEth(allowanceOwnerAlice), 2000);

      console.log("\tDecrease it by 1000.");
      await distTypeE.connect(owner).decreaseAllowance(alice.address, ethToWei(1000));
      allowanceOwnerAlice = await distTypeE.allowance(owner.address, alice.address);
      console.log("\tallowance[owner][alice]: %s", weiToEthEn(allowanceOwnerAlice));
      expectEqual(weiToEth(allowanceOwnerAlice), 1000);
