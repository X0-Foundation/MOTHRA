...
const {
    deployWireLibrary, deployNovelTypeH,
    ...
    let integralMathLib, PCSTypeA, distTypeB, PCSTypeC, distTypeD, PCSTypeE, PCSTypeF, novelTypeH;
...
async function transfer(sender, recipient, amount) {
    ...
    //console.log("\t%s is transferring %s %s NT_H ...".yellow, sender.name, recipient.name, weiToEth(amountWei));
    console.log("\t%s is transferring %s NT_H to %s...".yellow, 
...
async function mint(minter, to, amount) {
    let amountWei = ethToWei(amount);
    await console.log("\t%s is minting %s NT_H to %s ...".yellow, 
...
async function burn(burner, from, amount) {
    ...
    amountWei = ethToWei(amount);
    await console.log("\t%s is burning %s NT_H from %s ...".yellow, 
...
describe("====================== Stage 1: Deploy ======================\n".yellow, async function () {
    it("1.1 Main contracts are deployed.\n".green, async function () {
        ...
        novelTypeH = await deployNovelTypeH(owner, analyticMath.address);
        console.log("\tNovelTypeH contract deployed at: %s", novelTypeH.address);
    ...
    it("1.2 DistTyepA token name, symbol and decimals were checked.\n".green, async function () {
      ...
      console.log("\tNovelTypeH token's name: %s", name);
      expectEqual(name, "NovelTypeH");

      const symbol = await novelTypeH.symbol();
      console.log("\tNovelTypeH symbol: %s", symbol);
      expectEqual(symbol, "NT_H");

      const decimals = await novelTypeH.decimals();
      console.log("\tNovelTypeH decimals: %s", decimals);
    ...
    it("1.3 Total supply and owner balance of NT_H are checked.\n".green, async function () {
      const totalSupply = await novelTypeH.totalSupply();
      console.log("\tNovelTypeH total supply: %s gways",BigInt(totalSupply));
      ...
      console.log("\tNovelTypeH owner balance: %s gways", BigInt(ownerTgrBalance));
    ...
    it("1.4 Allowance control functions were checked.\n".green, async function () {
      console.log("Owner approves Alice to spend 1000 NT_H");
